...
public class DRSKBest {
	...
	class KBestHeapKey implements Heapable {
		...
		private int pos;
		private Hash hsh;
		public KBestHeapKey(DerivationRule r, int[] c, double v, double kc) {
			val = v;
			kidcount = kc;
			rule = r;
			choices = c;
	...
	public DRSKBest(DerivationRuleSet ruleSet) throws UnusualConditionException {
		rs = ruleSet;
	...
	private void buildOneBest() throws UnusualConditionException {
		...
		do {
			currentCount = 0;
			stit = rs.getStates().iterator();
			while (stit.hasNext()) {
				int s = stit.next();
	//			if (debug) Debug.debug(debug, "Considering state "+s);
				boolean added = false;
				double currentBestScore = semiring.ZERO();
				HashSet currRules = rs.getRulesByLHS(s);
				if (currRules == null)
					continue;
				Iterator rit = currRules.iterator();
				DerivationRule currBestRule = null;
				while (rit.hasNext()) {
					DerivationRule r = (DerivationRule)rit.next();
//					if (debug) Debug.debug(debug, "\tConsidering rule "+r.toString());
	...
	private void getCandidates(int state) throws UnusualConditionException {
		...
		Semiring semiring = rs.getSemiring();
		HashSet currRules = rs.getRulesByLHS(state);
		if (currRules == null)
			return;
	...
	public TreeItem[] getKBestTrees(int k, boolean isStringCopy) throws UnusualConditionException {
		TreeItem[] ret = new TreeItem[k];
		for (int i = 1; i <= k; i++) {
			Vector v = new Vector();
			ret[i-1] = buildKBestTree(i, v, true, isStringCopy);
			if (ret[i-1] == null)
				break;
	...
	public TreeItem buildKBestTree(int k, Vector rset, boolean fillHash, boolean isStringCopy) throws UnusualConditionException {
		boolean debug = false;
		TreeItem seedTree = null;
		try {
			seedTree = new TreeItem(SymbolFactory.getSymbol(""+rs.getStartState()), rs.getSemiring().ONE());
			KBestHeapKey deriv = getKthBestDeriv(rs.getStartState(), k);
			if (deriv != null) {
				if (debug) Debug.debug(debug, k+"th best deriv is "+deriv.toString());
				buildKBestTreeDriver(seedTree, deriv, rset, fillHash, debug, isStringCopy, 0);
			}
			else
				seedTree = null;
		}
	...
	private void buildKBestTreeDriver(TreeItem t, KBestHeapKey deriv, 
			...
			boolean debug, boolean isStringCopy, int level) throws UnusualConditionException {
		...
		if (isStringCopy)
			t.deepCopyDerivRuleRHS(deriv.rule);
		else
			t.deepCopyDerivRule(deriv.rule);
		if (fillHash)
			rset.add(deriv.rule);
		...
		for (int i = 0; i < leaves.length; i++) {
			...
			try {
				...
				if (rs.getStates().contains(labelint)) {
					...
					buildKBestTreeDriver(leaves[i], getKthBestDeriv(labelint, deriv.choices[nextItem++]), rset, fillHash, debug, isStringCopy, level+1);
	...
	private double buildKBestAlignmentDriver(StringBuffer ret, KBestHeapKey deriv, 
		...
		for (int i = 0; i < deriv.rule.getNumChildren(); i++) {
			...
			weight = rs.getSemiring().times(weight, buildKBestAlignmentDriver(ret, getKthBestDeriv(leaves[i], deriv.choices[i]), debug, level+1));
		}
		return weight;
	}
	...
	private void calculateAlphas() {
		...
		while (!Q.isEmpty()) {
			...
			int x = currkey.key;
			HashSet currRules = rs.getRulesByLHS(x);
			Iterator rit = currRules.iterator();
	...
	// 	    DRSKBest kb = new DRSKBest(ruleSet);
	// 	    for (int i = 1; i <= 10; i++) {
