...
public class StateSeq implements StateSeqMember {
	...
	// factory memoizer
	static private Hashtable<EarleyState, Hashtable<Vector<Hashtable<Symbol, StateSeqMember>>, StateSeq>> table = new Hashtable<EarleyState, Hashtable<Vector<Hashtable<Symbol, StateSeqMember>>, StateSeq>>();
	...
	StateSeq(EarleyState e, TreeRuleTuple s, double k, double h) { 
...
//			table.put(e, new Hashtable<Vector<Hashtable<Symbol, StateSeqMember>>, StateSeq>());
//		if (!table.get(e).containsKey(sobj))
//			table.get(e).put(sobj, new StateSeq(e, ))
//			return table.
		es=e;
		known=k; heur=h; 
	}
	StateSeq(EarleyState e, double k, double h) {
		es=e;
		states = new Vector<Hashtable<Symbol, StateSeqMember>>();
		known = k;
		heur = h;
	}
	StateSeq(EarleyState e, Symbol scb, Vector<EarleyState> c, Vector<Vector<Symbol>> csv, double k, double h) {
		es=e;
		childVec = c;
		childStateChain = csv;
	...
	StateSeq(EarleyState e, StateSeq a, StateSeq b, Semiring s) {
	...
	StateSeq(EarleyState e, Hashtable<Symbol, StateSeqMember> table, double k, double h) {
		es=e;
		states = new Vector<Hashtable<Symbol, StateSeqMember>>();
		states.add(table);
		known = k;
	...
	StateSeq(StateSeq old, Vector<Hashtable<Symbol, StateSeqMember>> s) {
		es=old.es;
	...
	StateSeq(EarleyState e, Vector<Hashtable<Symbol, StateSeqMember>> s, Symbol scb, Vector<EarleyState> c, Vector<Vector<Symbol>> csv, double k, double h) {
		es=e;
		states = s;
		known = k;
		heur = h;
	...
	// remove n members from states and return the rest as a new StateSeq
	public StateSeq shift(int n) {
		Vector<Hashtable<Symbol, StateSeqMember>> news = new Vector<Hashtable<Symbol, StateSeqMember>>(states);
		for (int i = 0; i < n; i++)
			news.remove(0);
		return new StateSeq(this, news);
	}

	
	public String toString() { return states+":"+known+":"+heur; }

	// shouldn't get sym of a StateSeq with non-finished states, or more than one!
	public Symbol getSym() throws UnusualConditionException {
		...
		for (Hashtable<Symbol, StateSeqMember> tab : states) {
			...
			if (tab.size() > 0) {
				if (tab.get(SOLO) instanceof StateSeq)
					throw new UnusualConditionException("Requested symbol for incomplete state "+states);
