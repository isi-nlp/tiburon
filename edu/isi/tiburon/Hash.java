...
//    private static long buzhash (StringBuffer arg) /* Hash StringBuffer   */
    private static long buzhash (char[] arg) /* Hash char []   */
...
//    	StringBuffer tmp = new StringBuffer(8);
	...
	//	System.out.println("this at "+aval+", b at "+bval);
//        tmp.setLength(8);
        for( int i=0; i<4; i+=2 )
        {   //tmp.setCharAt( i,   (char) aval );
        	tmp[i] = (char) aval;
	    //	    System.out.println("char "+i+" is "+aval);
            aval >>>= 16;
//            tmp.setCharAt( i+1, (char) bval );
            tmp[i+1] = (char) bval;

	    //	    System.out.println("char "+(i+1)+" is "+bval);
            bval >>>= 16;
	}
	
        result.hash_code = buzhash( tmp );
	//	System.out.println("result at "+result.hash_code);
        return result;
        }
